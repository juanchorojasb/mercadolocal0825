    ...
    for (const archivo of archivosTiendas) {
      try {
        ...
        if (productosActivos.length > 0) {
          ...
          const categorias = [...new Set(productosActivos.map((p: any) => p.category).filter(Boolean) as string[])]
          ...
          const ultimaActualizacion = new Date(Math.max(...fechas.map((f: Date) => f.getTime()))).toISOString()
